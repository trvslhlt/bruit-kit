/** Renders a declarative list of form fields into a container -- meant for
 * an always-visible config panel rather than a popup, so a field's
 * presence in the layout should never depend on some OTHER field's
 * transient state. The one kind that needs that relationship (an override
 * checkbox gating a value control) is handled as a single compound
 * "override" field instead of two fields one of which conditionally
 * appears: the value control is always present, just disabled while not
 * overriding (see `renderOverrideField`).
 *
...
export type Field =
  ...
  | {
       ...
       * above them (see effectsFields.ts's own per-param rows), not a
       * general layout option every field needs. */
      indented?: boolean;
      /** Makes the label itself clickable -- e.g. effectsFields.ts's own
       * params open a small popup to edit this control's own min/max range
       * instead of a separate always-visible "Customize ranges" section
       * cluttering the panel with two extra rows per param. */
      onLabelClick?: () => void;
      /** Visual cue (label rendered in an accent color, left to the
       * caller's own CSS) that this control currently has a non-default
       * range set. Meaningless without onLabelClick. */
      labelCustomized?: boolean;
      /** Visual cue that this param is currently drifting/wandering on its
       * own (see effectsFields.ts's own enableDrift) -- a distinct color
       * from labelCustomized so "has a custom range" and "is actively
  ...
  | {
       ...
       * display-only values (e.g. a caller showing some other module's
       * current state read-only). onChange is still required by the type
       * but never called. */
      readOnly?: boolean;
      onChange: (value: string) => void;
    }
  | {
      ...
      kind: "button";
      onClick: () => void;
      /** The label embeds a user-given name (e.g. "Sample player: <sample
       * name>…") -- keeps that name's case intact against a caller's own
       * lowercase-everything styling, if it has one, instead of mangling
       * it. */
  ...
  | {
       ...
       * are worth being able to dial in *before* switching them on instead
       * of starting from scratch under time pressure; for those, set this
       * and pair it with a dimmed wrapper around the whole field instead
       * (purely visual -- the control stays interactive). */
  ...
  | {
      ...
      label: string;
      kind: "automation";
      /** A breakpoint curve (see createAutomationEditor) -- the whole
       * array is handed back on every drag/add/remove, not a single
       * value, so there's no separate "commit" event to split from a
       * live-preview one the way range/override fields do. */
  ...
  | {
      ...
      label: string;
      kind: "waveformRange";
      /** The decoded buffer to draw -- a caller typically only renders
       * this field once a sample is actually loaded, since there's
       * nothing to trim a range against before then. */
      buffer: AudioBuffer;
      range: WaveformRange;
